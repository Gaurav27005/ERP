...
const mongoose = require('mongoose'); // FIX: Added mongoose for ObjectId validation
...
router.post('/', auth, requireRole('tpo','admin','faculty'), async (req, res) => {
  try {
    ...
    // FIX: Prevent past date scheduling (Audit Issue #9)
    ...
    if (dateObj < today) {
      return res.status(400).json({ success:false, message:'Cannot schedule an interview in the past.' });
    }
...
router.put('/:id', auth, requireRole('faculty','admin','tpo'), async (req, res) => {
  try {
    ...
    if (req.body.scheduledDate) {
      ...
      if (!isNaN(d.getTime())) safeUpdate.scheduledDate = d;
    }
...
router.post('/:id/feedback', auth, requireRole('faculty','tpo','admin'), async (req, res) => {
  try {
    ...
    // FIX: Restrict feedback submission to the explicitly assigned interviewer (Audit Issue #20)
    if (req.user.role !== 'admin' && iv.interviewer?.toString() !== req.user._id.toString()) {
      return res.status(403).json({ success: false, message: 'Only the assigned interviewer can submit feedback for this session.' });
    }
